...
⚠️ **E repetiu-se na app a 2026-08-08**, apanhado a preencher a tabela de reverts do PR: o `fila.test.ts` dizia em comentário que a reversão era tirar a `filaDosBancos` do `queryFn`, e nessa montagem os 87 testes passavam e o `tsc` também. O que estava preso era a fila; o **fio** entre ela e o pedido não estava — a peça cuja falta faz sair cinco pedidos de uma vez para cinco bancos. Fechado pelo `fan-out.test.tsx`, que conta chamadas ao `fetch` e não tarefas. ⚠️ **Foi correr os reverts que o encontrou, e não escrevê-los** — uma tabela preenchida por argumento não o teria apanhado.
