...
2. **Confirmar a app contra o servidor** — a confirmação da A5 (2026-07-29) é anterior à `KAN-45` e à `KAN-50`, que mudaram quem aparece na lista, e à `KAN-51`, que mudou o que se lê no detalhe. ⚠️ A `KAN-51` não tocou no `api/`: o `sincronizar-api` e o travão do CI não vêem diferença nenhuma, e o ecrã que mostra os `pressupostos` por inteiro (`app/ofertas/[banco].tsx`) muda sem que nada avise.
